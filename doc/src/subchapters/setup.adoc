...
At this time, the webapp was created with the following:
...
To run the web app locally, change to the `webapp` directory, run `npm install` and afterwards `ng serve`.
Additional information can be found in the `README.md` file in the `webapp` directory.

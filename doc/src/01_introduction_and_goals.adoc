...
== Why are we developing Flagforge?

The idea of Flagforge is simple.
...
_Flagforge_ aims to solve this issue by abstracting away this functionality.
...
The following two diagrams demonstrate the rough idea/concept behind Flagforge.
They are by no means final or to be taken as correct.
...
....
graph TB
    subgraph "Flagforge Platform"
        Dashboard[["🖥️ Flagforge Dashboard"]]
        API[["🔌 Flagforge API"]]
...
sequenceDiagram
    ...
    participant D as Flagforge Dashboard
    participant DB as Flagforge Database
    participant App as Customer App
    participant API as Flagforge API
...
EaseOfUse:: Flagforge is not only catered to the developper and should also be easily configurable for non technical person to create feature sets and administering set sets
